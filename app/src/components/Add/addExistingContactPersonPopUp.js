...
import ExistingPersonTable from '../data-tables/existing-person-table.js';
...
class AddSpecialOrderForm extends React.Component {
  ...
  async componentDidMount() {
    ...
    await this.setState({ persons: response.allPersons });
    //this.getSchema();
  }
  ...
  handleAdd = async (person) => {
    ...
    if (clientResponse.ClientFound.contactPersons == undefined) {
      let res2 = await this.props.editClient(this.props.currentClient, {
        contactPersons: [person._id],
      });
      this.props.handleClose();
    } else {
             let response = await this.props.getClient(this.props.currentClient);
             console.log('bilbo');
             let currentContactPersons = clientResponse.ClientFound.contactPersons;
             await currentContactPersons.push(person._id);
  ...
  handleAddContractor = async (person) => {
    ...
    if (contractorResponse.ContractorFound.contactPersons == undefined) {
      ...
      this.props.handleClose();
             ...
             let response = await this.props.getContractor(this.props.currentContractor);
             console.log('bilbo');
      ...
      <div style={{ flexGrow: 1 }}>
        ...
        >
          ...
          <ExistingPersonTable
            ...
            handleAddContractor={this.handleAddContractor}
          />
          <DialogActions>
            <Button
              onClick={() => {
